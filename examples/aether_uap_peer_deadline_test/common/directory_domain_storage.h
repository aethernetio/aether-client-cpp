...
#ifndef EXAMPLES_AETHER_UAP_PEER_DEADLINE_TEST_COMMON_DIRECTORY_DOMAIN_STORAGE_H_
#define EXAMPLES_AETHER_UAP_PEER_DEADLINE_TEST_COMMON_DIRECTORY_DOMAIN_STORAGE_H_
...
class DirectoryDomainStorage final : public IDomainStorage {
 public:
  ...
  DomainLoad Load(DomainQuery const& query) override {
    ...
    return {DomainLoadResult::kLoaded, std::make_unique<Reader>(std::move(f))};
  }
  ...
  void CleanUp() override {
    // Preserve state across Bob hard-kill / restart so the same UID remains.
  }
...
#endif  // EXAMPLES_AETHER_UAP_PEER_DEADLINE_TEST_COMMON_DIRECTORY_DOMAIN_STORAGE_H_
