...
void test_OffsetRange() {
  constexpr auto half_window_range =
      OffsetRange{SSRingIndex{25}, SSRingIndex{75}};

  TEST_ASSERT_TRUE(half_window_range.IsAfter(SSRingIndex{24}));
  TEST_ASSERT_FALSE(half_window_range.IsAfter(SSRingIndex{25}));
  TEST_ASSERT_FALSE(half_window_range.IsAfter(SSRingIndex{26}));

  TEST_ASSERT_TRUE(half_window_range.IsBefore(SSRingIndex{76}));
  TEST_ASSERT_FALSE(half_window_range.IsBefore(SSRingIndex{75}));
  TEST_ASSERT_FALSE(half_window_range.IsBefore(SSRingIndex{74}));
  ...
  constexpr auto window_range = OffsetRange{SSRingIndex{0}, SSRingIndex{100}};
  ...
  TEST_ASSERT_TRUE(window_range.IsBefore(SSRingIndex{110}));
  TEST_ASSERT_FALSE(window_range.IsAfter(SSRingIndex{110}));
}
