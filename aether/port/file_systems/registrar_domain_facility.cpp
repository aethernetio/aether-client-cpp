...
#if defined REGISTRAR_DOMAIN_FACILITY_ENABLED
...
RegistrarDomainFacility::RegistrarDomainFacility(
...
std::vector<uint32_t> RegistrarDomainFacility::Enumerate(const ObjId& obj_id) {
...
void RegistrarDomainFacility::Store(const ObjId& obj_id, std::uint32_t class_id,
                                    std::uint8_t version,
...
void RegistrarDomainFacility::Load(const ObjId& obj_id, std::uint32_t class_id,
                                   std::uint8_t version,
...
void RegistrarDomainFacility::Remove(const ObjId& obj_id) {
  auto it = state_.find(obj_id);
  if (it != state_.end()) {
...
void RegistrarDomainFacility::CleanUp() { state_.clear(); }
#  endif

void RegistrarDomainFacility::SaveState() {
...
void RegistrarDomainFacility::PrintData(std::ofstream& file,
...
void RegistrarDomainFacility::PrintMapKeysAsData(std::ofstream& file,
