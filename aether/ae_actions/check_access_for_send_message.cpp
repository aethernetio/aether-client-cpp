...
void CheckAccessForSendMessage::SendRequest(TimePoint current_time) {
  ...
  auto packet = PacketBuilder{
      ...
      PackMessage{AuthorizedApi{}, AuthorizedApi::CheckAccessForSendMessage{
                                       {}, request_id, destination_}}};

  SendResult::OnResponse(client_to_server_stream_->protocol_context(),
                         request_id, [&](auto&) { ResponseReceived(); });
  SendError::OnError(client_to_server_stream_->protocol_context(), request_id,
