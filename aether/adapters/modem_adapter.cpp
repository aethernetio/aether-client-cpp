...
ModemAdapter::ModemAdapter(ObjPtr<Aether> aether, 
                           IPoller::ptr poller,
                           ModemInit modem_init, 
                           Domain* domain)
    : ParentModemAdapter{std::move(aether), poller,
                         modem_init, domain} {
  modem_driver_ = ModemDriverFactory::CreateModem(*this, poller, modem_init, domain);
  modem_driver_->Init();
