...
using ClientListener = SmallFunction<EventHandlerDeleter(
    ClientApiSafe& client_api, ServerConnection* server_connection)>;

// call authorized api
using AuthApiCaller = SmallFunction<void(ApiContext<AuthorizedApi>& auth_api,
                                         ServerConnection* server_connection)>;
// make request to authorized api
using AuthApiRequest = SmallFunction<void(ApiContext<AuthorizedApi>& auth_api,
                                          ServerConnection* server_connection,
                                          CloudRequestAction* request)>;

// listen to client's api response
using ClientResponseListener = SmallFunction<EventHandlerDeleter(
    ClientApiSafe& client_api, ServerConnection* server_connection,
    CloudRequestAction* request)>;
