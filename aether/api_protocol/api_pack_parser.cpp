...
ApiParser::ApiParser(ProtocolContext& protocol_context,
    ...
    : protocol_context_{protocol_context},
      archive{MessageBuffer{
          const_cast<std::vector<std::uint8_t>&>(data),  // NOLINT(*const-cast)
      }} {
...
void ApiParser::Cancel() {
  archive.buffer().read_offset = archive.buffer().buff.size();
}

ApiPacker::ApiPacker(ProtocolContext& protocol_context_,
                     std::vector<std::uint8_t>& data)
    : protocol_context_{protocol_context_}, archive{MessageBuffer{data}} {
