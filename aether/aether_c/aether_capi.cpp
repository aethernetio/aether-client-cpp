...
struct AetherClient {
  ...
  ae::OwnActionPtr<ae::PipelineQueue> pipeline_queue_;
};
...
AetherClient* SelectClient(ClientConfig const* config) {
  ...
  ret_client->pipeline_queue_ =
      ae::OwnActionPtr<ae::PipelineQueue>{*aether_app};

  ret_client->pipeline_queue_->Push(ae::Stage(
